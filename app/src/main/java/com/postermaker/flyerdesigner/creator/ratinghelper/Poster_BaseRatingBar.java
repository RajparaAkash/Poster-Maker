...
public class Poster_BaseRatingBar extends LinearLayout implements Poster_SimpleRatingBar {

    public interface OnRatingChangeListener {
        void onRatingChange(Poster_BaseRatingBar ratingBar, float rating);
    }
    ...
    protected List<Poster_PartialView> mPartialViews;

    public Poster_BaseRatingBar(Context context) {
        this(context, null);
    }

    /* Call by xml layout */
    public Poster_BaseRatingBar(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Poster_BaseRatingBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void verifyParamsValue() {
        ...
        if (mEmptyDrawable == null) {
            mEmptyDrawable = ContextCompat.getDrawable(getContext(), R.drawable.poster_empty);
        }

        if (mFilledDrawable == null) {
            mFilledDrawable = ContextCompat.getDrawable(getContext(), R.drawable.poster_filled);
        }
    ...
    private void initRatingView() {
        ...
        for (int i = 1; i <= mNumStars; i++) {
            Poster_PartialView partialView = getPartialView(i, mFilledDrawable, mEmptyDrawable);
            mPartialViews.add(partialView);
            addView(partialView, params);
        }
    }

    private Poster_PartialView getPartialView(final int ratingViewId, Drawable filledDrawable, Drawable emptyDrawable) {
        Poster_PartialView partialView = new Poster_PartialView(getContext());
    ...
    protected void fillRatingBar(final float rating) {
        for (Poster_PartialView partialView : mPartialViews) {
